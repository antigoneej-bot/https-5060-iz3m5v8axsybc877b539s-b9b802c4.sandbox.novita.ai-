...
class _PeriodReflectionCardState extends State<PeriodReflectionCard> {
  ...
  void initState() {
    ...
    _notes = List.generate(
      3,
      (i) => DraftTextController('reflection_${widget.periodKey}_$i'),
    );
  }
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              widget.monthly ? '나의 월간 회고' : '나의 주간 회고',
              style: titleFont(fontSize: 26, color: AppColors.titlePastelGreen),
            ),
            const SizedBox(height: 8),
            Text(
              '한 줄만 남겨도 좋아요. 이전 주·달로 이동하면 다시 볼 수 있어요.',
            ...
            for (var i = 0; i < prompts.length; i++) ...[
              ...
              Text(
                prompts[i],
                style: bodyFont(fontSize: 14, fontWeight: FontWeight.w700),
              ),
              const SizedBox(height: 8),
              TextField(
                ...
                decoration: InputDecoration(
                  hintText: '여기에 마음을 남겨 주세요',
                  counterText: '',
                  contentPadding: const EdgeInsets.all(16),
                ),
              ),
              ValueListenableBuilder<String>(
                valueListenable: _notes[i].saveStatus,
                builder: (_, status, __) => Text(
                  status.replaceAll('임시저장', '저장'),
            ...
            Text(
              '회고도 백업에 포함돼요. 앱 삭제에 대비해 별도 백업을 보관해 주세요.',
