...
class BackupService {
  ...
  static const globals = {
    ...
    'welcome_intro_completed_v1',
    'cat_browse_favorites_v1',
    'bgm_enabled',
    'sfx_enabled',
    'bgm_volume',
  };
  static String? prefType(String key) {
    const globalTypes = {
      'mongi_last_emotions': 'strings',
      'mongi_last_intensity': 'double',
      'companion_name': 'string',
      'onboarding_completed_v1': 'bool',
      'welcome_intro_completed_v1': 'bool',
      'cat_browse_favorites_v1': 'strings',
      'bgm_enabled': 'bool',
      'sfx_enabled': 'bool',
      'bgm_volume': 'double',
    };
    ...
    const ints = {
      ...
      'care_growth_days',
      'care_points',
      'care_total_points_earned',
      'care_total_full_care_days',
      'care_total_consumables_used',
      'care_total_pats',
      'streak_count',
      'growth_level',
    };
    const strings = {
      'mongi_garden_v1',
      'care_last_date',
      'care_companion_cat_id',
      'care_attendance_counted_date',
      'daily_card_last_date',
      'daily_card_last_cat_id',
      'bubble_last_date',
      'last_visit_date',
      'growth_challenge_start',
    };
    const lists = {
      'care_temp_history',
      'care_owned_accessories',
      'care_equipped_slots',
      'care_consumable_inventory',
      'care_graduated_cats',
      'care_unlocked_achievements',
      'bubble_session_cat_ids',
      'bubble_popped_indices',
      'growth_completed_days',
      'journey_done',
    };
    const bools = {
      'care_fed_today',
      ...
      'care_bathed_today',
      'care_cleaned_today',
      'care_breathing_today',
      'care_walking_today',
      'care_journaling_today',
      'care_gratitude_today',
    };
    if (ints.contains(name) ||
        RegExp(
          r'^care_activity_bonus_claimed_\d{4}-\d{2}-\d{2}$',
        ).hasMatch(name))
      return 'int';
    if (bools.contains(name) ||
        RegExp(r'^care_bonus_given_\d{4}-\d{2}-\d{2}$').hasMatch(name))
      return 'bool';
  ...
  static Future<Box?> _existing(String name) async {
    ...
    if (!await Hive.boxExists(logical) &&
        !await Hive.boxExists('${logical}__enc'))
      return null;
    ...
    for (final name in boxes) {
      ...
      data[name] = [
        for (final key in box.keys.toList())
          if (name != 'mongi_progress' || MongiBackupSchema.allows(key))
            {'key': key, 'value': box.get(key)},
      ];
    }
  ...
  static void validate(Map<String, dynamic> snapshot) {
    if (snapshot['schema'] != 1 ||
        snapshot['boxes'] is! Map ||
        snapshot['settings'] is! Map ||
        DateTime.tryParse(snapshot['createdAt']?.toString() ?? '') == null)
      throw const FormatException('백업 구조가 올바르지 않아요.');
    final catIds = shadowCats.map((cat) => cat.id).toSet();
    var total = 0;
    for (final entry in (snapshot['boxes'] as Map).entries) {
      if (!boxes.contains(entry.key) || entry.value is! List)
        throw const FormatException('알 수 없는 기록 종류예요.');
      final seen = <Object>{};
      for (final row in entry.value as List) {
        if (++total > 50000 ||
            row is! Map ||
            (row['key'] is! String && row['key'] is! int) ||
            !seen.add(row['key'] as Object))
          throw const FormatException('잘못된 기록 항목이에요.');
        ...
        if (entry.key == 'personal_replies') {
          if (row['key'] is String &&
              (row['key'] as String).startsWith('quota:')) {
            final day = (row['key'] as String).substring(6);
            if (!RegExp(r'^\d{4}-\d{2}-\d{2}$').hasMatch(day) ||
                DateTime.tryParse(day) == null ||
                value is! int ||
                value < 0 ||
                value > 1) {
              throw const FormatException('잘못된 답장 이용 기록이에요.');
            }
            continue;
          }
          if (row['key'] is! String ||
          ...
          if (metadata is! Map ||
              metadata['version'] != 1 ||
              metadata['id'] != row['key'] ||
              metadata['reply'] is! String ||
              metadata['parts'] is! List ||
        ...
        if (entry.key == 'garden_memories') {
          ...
          if (row['key'] != memory.plantId)
            throw const FormatException('식물 기록이 일치하지 않아요.');
          continue;
        }
        if (entry.key == 'reply_feedback') {
          if (row['key'] is! String ||
              !{'matched', 'off_topic', 'repeated'}.contains(value))
            throw const FormatException('잘못된 답장 평가예요.');
          continue;
        }
        if (entry.key == 'drafts' || entry.key == 'reply_cache') {
          if (value is! String || value.length > 100000)
            throw const FormatException('잘못된 편지예요.');
          continue;
        }
        if (value is! Map) throw const FormatException('잘못된 기록이에요.');
        if (value.containsKey('catId') && !catIds.contains(value['catId']))
          throw const FormatException('지원하지 않는 고양이 기록이에요.');
        switch (entry.key) {
          case 'letter_entries':
            LetterEntry.fromMap(value);
            break;
          case 'reflection_letters':
            ReflectionLetterEntry.fromMap(value);
            break;
          case 'daily_draw_entries':
            DailyDrawEntry.fromMap(value);
            break;
          case 'bubble_memo_entries':
            BubbleMemoEntry.fromMap(value);
            break;
          case 'buried_emotions':
            BuriedEmotionEntry.fromMap(value);
            break;
          case 'cat_memories':
            MemoryEntry.fromMap(value);
            break;
          case 'usage_history':
            UsageHistoryEntry.fromMap(value);
            break;
          case 'promise_entries':
            PromiseEntry.fromMap(value);
            break;
          case 'special_letters':
            SpecialLetterEntry.fromMap(value);
            break;
        }
      }
    }
    for (final entry in (snapshot['settings'] as Map).entries) {
      if (entry.key is! String ||
          !allowedPref(entry.key as String) ||
          entry.value is! Map)
        throw const FormatException('허용되지 않은 설정이에요.');
      ...
      final valid = switch (entry.value['type']) {
        ...
        'double' => v is num,
        'string' => v is String,
        'strings' => v is List && v.every((e) => e is String),
        _ => false,
      };
      if (!valid || entry.value['type'] != prefType(entry.key as String))
        throw const FormatException('설정 값이 올바르지 않아요.');
  ...
  static int count(Map<String, dynamic> data) => (data['boxes'] as Map).values
  ...
  static Future<int> restore(Map<String, dynamic> data) async {
    ...
    if (journal.containsKey('pending') || journal.containsKey('deferred'))
      throw StateError('이전 복원을 마치려면 앱을 다시 열어 주세요.');
    await journal.put('pending', {
      'data': data,
      'restoreProfile': !await hasLocalRecords(),
    });
    await journal.flush();
  ...
  static Future<void> deferPending() async {
    ...
    if (journal.containsKey('deferred') &&
        jsonEncode(journal.get('deferred')['data']) !=
            jsonEncode(pending['data'])) {
      throw StateError('다른 보류 복원이 이미 있어요.');
    }
    await journal.put('deferred', {
      'data': pending['data'],
  ...
  static Future<void> archiveDeferred() async {
    ...
    if (journal.containsKey('pending'))
      throw StateError('진행 중인 복원이 있어요. 앱을 다시 열어 보류해 주세요.');
    final deferred = journal.get('deferred');
    if (deferred == null) return;
    await journal.put(
      'archived_${DateTime.now().microsecondsSinceEpoch}',
      deferred,
    );
    await journal.flush();
  ...
  static Future<bool> hasPending() async =>
      (await _journal()).containsKey('pending');
  static Future<bool> hasDeferred() async =>
      (await _journal()).containsKey('deferred');
  ...
  static Future<int> resumePending() async {
    ...
    for (final entry in (data['boxes'] as Map).entries) {
      ...
      for (final row in entry.value as List) {
        if (!box.containsKey(row['key'])) {
          await box.put(row['key'], row['value']);
          added++;
        }
      }
      await box.flush();
    ...
    for (final entry in (data['settings'] as Map).entries) {
      ...
      if (pending['restoreProfile'] != true && prefs.containsKey(entry.key))
        continue;
      final v = entry.value['value'];
      ...
      final stored = switch (entry.value['type']) {
        ...
        'strings' => await prefs.setStringList(
          key,
